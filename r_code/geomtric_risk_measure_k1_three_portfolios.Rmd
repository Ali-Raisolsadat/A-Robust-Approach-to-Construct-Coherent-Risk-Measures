...
# Put the portfolio values in order and remove NA values
...
second_portfolio_df <- tail(portfolio_df, n_paths - (n_paths * alpha_values[1]))
first_minimum_df <- second_portfolio_df[second_portfolio_df$Portfolio_Value_T == min(second_portfolio_df$Portfolio_Value_T), ]

third_portfolio_df <- tail(portfolio_df, n_paths - (n_paths * alpha_values[2]))
second_minimum_df <- third_portfolio_df[third_portfolio_df$Portfolio_Value_T == min(third_portfolio_df$Portfolio_Value_T), ]

fourth_portfolio_df <- tail(portfolio_df, n_paths - (n_paths * alpha_values[3]))
third_minimum_df <- fourth_portfolio_df[fourth_portfolio_df$Portfolio_Value_T == min(fourth_portfolio_df$Portfolio_Value_T), ]
...
intercept <- data.frame(
  y_values = seq(from = as.numeric(lower_intercept$Portfolio_Value_T) - 5, to = as.numeric(upper_intercept$Portfolio_Value_T), length.out = n_steps),
  x_values_1 = rep(k1, n_steps),
  x_values_2 = rep(as.numeric(second_portfolio_df$Min_Distances[1]), n_steps),
  x_values_3 = rep(as.numeric(third_portfolio_df$Min_Distances[1]), n_steps),
  x_values_4 = rep(as.numeric(fourth_portfolio_df$Min_Distances[1]), n_steps)
)

# Create data frames for the intercept lines
intercept_lines <- data.frame(
  ...
  alpha = factor(rep(alpha_values, each = length(intercept$x_values_2)))
)

# Create the initial scatter plot for portfolio values
p <- ggplot(portfolio_df, aes(x = Min_Distances, y = Portfolio_Value_T)) +
...
# Add intercept lines for each alpha value with color
p <- p + geom_line(data = intercept_lines, aes(x = x, y = y, color = alpha), size = 1) +
  scale_color_manual(values = c("blue", "green", "red"), 
                     labels = c(TeX(sprintf(r'($\alpha = %f$)', alpha_values))),
                     name = "Alpha Values")

# Highlight and annotate the first minimum points
p <- p + geom_point(data = first_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T), color = "blue", size = 3) +
  geom_label(data = first_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T - 40, label = paste("risk measure =", round(Portfolio_Value_T, 2))), 
             size = 3, color = "blue", fill = "lightblue", label.size = 0.25, vjust = -1)

# Highlight and annotate the second minimum points
p <- p + geom_point(data = second_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T), color = "green", size = 3) +
  geom_label(data = second_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T - 30, label = paste("risk measure =", round(Portfolio_Value_T, 2))), 
             size = 3, color = "green", fill = "pink", label.size = 0.25, vjust = -1)

# Highlight and annotate the third minimum points
p <- p + geom_point(data = third_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T), color = "red", size = 3) +
  geom_label(data = third_minimum_df, aes(x = Min_Distances, y = Portfolio_Value_T - 20, label = paste("risk measure =", round(Portfolio_Value_T, 2))), 
             size = 3, color = "red", fill = "lightgrey", label.size = 0.25, vjust = -1)

...
#' Find Minimum Points After Discarding Alpha Values
#'
#' This function discards a specified proportion (`alpha`) of the smallest distances
#' and then finds the row with the minimum portfolio value.
#'
#' @param df A data frame containing portfolio values and distances.
#' @param alpha A numeric value representing the proportion of rows to discard.
#' @return A data frame containing the row with the minimum portfolio value after discarding.
find_minimum_df <- function(df, alpha) {
  n_discard <- ceiling(nrow(df) * alpha)
  tail_df <- tail(df, nrow(df) - n_discard)
  tail_df[tail_df$Portfolio_Value_T == min(tail_df$Portfolio_Value_T), ]
}

#' Highlight and Annotate Minimum Points for Each Alpha Value
#'
#' This function adds points and labels to a ggplot object to highlight and annotate
#' the minimum portfolio values for each alpha value.
#'
#' @param plot A ggplot object to which the points and labels will be added.
#' @param df A data frame containing the points to be highlighted and annotated.
#' @param color A character string representing the color of the points and labels.
#' @param label_y_offset A numeric value specifying the vertical offset for the labels.
#' @return A ggplot object with the added points and labels.
highlight_and_annotate <- function(plot, df, color, label_x_offset, label_y_offset) {
  plot + 
    geom_point(data = df, aes(x = Min_Distances, y = Portfolio_Value_T), color = color, size = 3) +
    geom_label(data = df, aes(x = Min_Distances + label_x_offset, y = Portfolio_Value_T + 
                                label_y_offset, label = paste("risk measure =", round(Portfolio_Value_T, 2))), 
               size = 3, color = color, fill = "lightblue", label.size = 0.25, vjust = -1)
}

#' Plot Portfolio Values vs. Minimum Distances
#'
#' This function creates a scatter plot of portfolio values vs. minimum distances, 
#' adds intercept lines for specified alpha values with different line types, and highlights and annotates
#' the minimum points for each alpha value.
#'
#' @param portfolio_df A data frame containing the portfolio values and minimum distances.
#' @param intercept_lines A data frame containing the intercept lines for the plot.
#' @param first_minimum_df A data frame containing the first minimum points to be highlighted and annotated.
#' @param second_minimum_df A data frame containing the second minimum points to be highlighted and annotated.
#' @param third_minimum_df A data frame containing the third minimum points to be highlighted and annotated.
#' @param alpha_values A numeric vector of alpha values.
#' @return A ggplot object displaying the plot.
#' @import ggplot2
#' @importFrom ggtext TeX
plot_portfolio_values <- 
  function(portfolio_df, intercept_lines, first_minimum_df, second_minimum_df, third_minimum_df, alpha_values, type="long") {
    if(type == "short") {
      x_offset_val = -0.1
      y1_offset_val = -3
      y2_offset_val = -4.25
    ...
    p <- 
      ggplot(portfolio_df, aes(x = Min_Distances, y = Portfolio_Value_T)) +
      geom_point(color = "black", alpha = 0.5, size = 1.5) +
      labs(x = expression(M(t) == min(f(t) - g(t))), y = expression(V[phi](omega, T)),
           title = expression(Portfolio~Value~V[phi](omega, T)~vs.~Minimum~Distance~M(t))) +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5))
    
    # Add intercept lines for each alpha value with different line types
    p <- p + 
      geom_line(data = intercept_lines, aes(x = x, y = y, color = alpha, linetype = alpha), size = 1) +
      scale_color_manual(values = c("darkgreen", "darkblue", "orangered"), 
                         labels = c(TeX(sprintf(r'($\alpha = %.3f$)', alpha_values[1])),
                                    TeX(sprintf(r'($\alpha = %.3f$)', alpha_values[2])),
                                    TeX(sprintf(r'($\alpha = %.3f$)', alpha_values[3]))),
                         name = "Alpha Values") +
    ...
    p <- highlight_and_annotate(p, first_minimum_df, "darkgreen", x_offset_val, y3_offset_val)
    p <- highlight_and_annotate(p, second_minimum_df, "darkblue", x_offset_val, y2_offset_val)
    p <- highlight_and_annotate(p, third_minimum_df, "orangered", x_offset_val, y1_offset_val)
    
    # Add legend
    p <- p + theme(legend.position = "right") +
      guides(color = guide_legend(title = TeX('$\\alpha$ Values')),
             linetype = guide_legend(title = TeX('$\\alpha$ Values')))
...
for (i in seq_along(rho)) {
  ...
  # Compute the delta hedged portfolio for the option
...
# Average realized volatility from the CIR process simulations
average_realized_volatility <- rowMeans(sqrt(cir_vol_paths))
...
log_of_asset_prices <- log1p(s_paths_gbm)  # Log transformation of asset prices

# Find the minimum distance. If it is negative then they are below (threshold - k1), so we remove them
min_distances <- apply(log_of_asset_prices - (threshold_r_t - k1), 1, min)
min_distances[min_distances < 0] <- NA
...
# Ensure that the distances are set to NA if corresponding portfolio value is NA
min_distances[is.na(portfolio_value_T)] <- NA

# Create data frame to store calculated values
...
percent_omitted <- (sum(is.na(portfolio_df$Portfolio_Value_T)) / n_paths) * 100

# Calculate new risk measure value
new_risk_measure_value <- (-1) * min(portfolio_df$Portfolio_Value_T, na.rm = TRUE)
cat(sprintf('The full time risk measure for buy-and-hold position is %.5f with correlation value of %.2f and K1 = %.2f\n',
            new_risk_measure_value, rho, single_k1_threshold))

# Calculate example 3 new risk measure value
ex3_risk_measure_value <- (-1) * min(portfolio_df$Ex3_Risk_Measure, na.rm = TRUE)
cat(sprintf('The half time risk measure for buy-and-hold position is %.5f with correlation value of %.2f and K1 = %.2f\n',
            ex3_risk_measure_value, rho, single_k1_threshold))

# Order portfolio_df by Min_Distances and remove NA values
portfolio_df <- portfolio_df[!is.na(portfolio_df$Portfolio_Value_T), ]
portfolio_df <- portfolio_df[order(portfolio_df$Min_Distances), ]


# Find minimum points for each alpha value
first_minimum_df <- find_minimum_df(portfolio_df, alpha_values[1])
second_minimum_df <- find_minimum_df(portfolio_df, alpha_values[2])
third_minimum_df <- find_minimum_df(portfolio_df, alpha_values[3])

# Create intercept lines
intercept <- data.frame(
  y_values = seq(from = min(portfolio_df$Portfolio_Value_T) - 5, to = max(portfolio_df$Portfolio_Value_T), length.out = n_steps),
  x_values_1 = rep(k1, n_steps),
  x_values_2 = rep(first_minimum_df$Min_Distances[1], n_steps),
  x_values_3 = rep(second_minimum_df$Min_Distances[1], n_steps),
  x_values_4 = rep(third_minimum_df$Min_Distances[1], n_steps)
)

intercept_lines <- data.frame(
  x = c(intercept$x_values_2, intercept$x_values_3, intercept$x_values_4),
  y = c(intercept$y_values, intercept$y_values, intercept$y_values),
  alpha = factor(rep(alpha_values[1:3], each = length(intercept$y_values)))
)

# Create the scatter plot for portfolio values and distinguish different new risk measures for different alpha levels
plot_portfolio_values(portfolio_df, intercept_lines, first_minimum_df, second_minimum_df, third_minimum_df, alpha_values)
```
...
# Compute the portfolio value at time T for each path
portfolio_value_T <- long_call_portfolio_payoff_T(hedged_p=call_delta_hedged_prices, call_p=call_option_prices, r=risk_free_rate, t=T)
value_of_portfolio_T_2 <- 
  long_call_portfolio_payoff_T_2(hedged_p = call_delta_hedged_prices, call_p = call_option_prices, r = risk_free_rate, t = T, n_steps = n_steps)
...
portfolio_value_T <- short_call_portfolio_payoff_T(hedged_p = call_delta_hedged_prices, call_p = call_option_prices, t = n_steps)
value_of_portfolio_T_2 <- 
  short_call_portfolio_payoff_T_2(hedged_p = call_delta_hedged_prices, call_p = call_option_prices, t = n_steps)
...
# Create intercept lines
intercept <- data.frame(
  y_values = seq(from = lower_bound, to = upper_bound, length.out = n_steps),
  x_values_1 = rep(k1, n_steps),
  x_values_2 = rep(first_minimum_df$Min_Distances[1], n_steps),
  x_values_3 = rep(second_minimum_df$Min_Distances[1], n_steps),
  x_values_4 = rep(third_minimum_df$Min_Distances[1], n_steps)
...
# Create the scatter plot for portfolio values and distinguish different new risk measures for different alpha levels
plot_portfolio_values(portfolio_df, intercept_lines, first_minimum_df, second_minimum_df, third_minimum_df, alpha_values, type = "short")
```
